...
def create_test_model(
    name: str = "test",
    base: Type[pydantic.BaseModel] = BaseSettings,
...
def create_test_field(
    ...
    """Construct a `pydantic` FieldInfo with sensible defaults for testing.

    Args:
        name (str): Name used to construct an internal throwaway model.
        type_ (Type[Any]): Annotation/type for the field.
        default (Any): Default value for the field. Use `...` for required.
        description (Optional[str]): Description for the field.

    Returns:
        pydantic.fields.FieldInfo: The constructed FieldInfo instance.
    """
    # Create a throwaway dynamic model with a single field named `value`
    DynamicModel = pydantic.create_model(
        f"TestFieldModel_{name}",
        value=(type_, pydantic.Field(default, description=description, alias=name)),
        __base__=pydantic.BaseModel,
    )

    # Pydantic v2: fields live on `model_fields`
    return DynamicModel.model_fields["value"]
...
class TestModel(pydantic.BaseModel):
    ...
    arg_24: List[str] = pydantic.Field(
        default_factory=lambda: ["A", "B", "C"],
        description="arg_24",
    )
    arg_25: Tuple[str, str, str] = pydantic.Field(
        ("A", "B", "C"),
        description="arg_25",
    )
    arg_26: Set[str] = pydantic.Field(
        default_factory=lambda: {"A", "B", "C"},
        description="arg_26",
    )
    arg_27: FrozenSet[str] = pydantic.Field(
        default_factory=lambda: frozenset(("A", "B", "C")),
        description="arg_27",
    )
    arg_28: Deque[str] = pydantic.Field(
        default_factory=lambda: collections.deque(("A", "B", "C")),
        description="arg_28",
    )
    arg_29: Dict[str, int] = pydantic.Field(
        default_factory=lambda: {"A": 123},
        description="arg_29",
    )
    ...
    arg_31: datetime.datetime = pydantic.Field(
        datetime.datetime(2021, 12, 25, 7),
        description="arg_31",
    )
    arg_32: datetime.time = pydantic.Field(
        datetime.time(7, 30),
        description="arg_32",
    )
