## Overview
At the core of `pydanticV2-argparse` is the `pydantic` *model*, in which
...
`pydantic.BaseModel` base class. In `pydanticV2-argparse`, this model is used to
...
These fields define each `pydanticV2-argparse` command-line argument, and they
...
### Under the Hood
Under the hood `pydanticV2-argparse` dynamically generates *extra* custom
...
!!! note
    `pydanticV2-argparse` also enhances `pydantic`'s built-in
    ...
    means the values never reach the generated `pydanticV2-argparse` validators,
    ...
    `pydanticV2-argparse` validators and the built-in `pydantic` field validators
    if applicable.
